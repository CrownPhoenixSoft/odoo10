...
class AbstractReportXlsx(ReportXlsx):
    ...
    def _write_line(self, ws, row_pos, ws_params, col_specs_section=None,
                    render_space=None,count=None, default_format=None,):
        """
        ...
        if count == 1:
            col_specs = ws_params.get('col_specs_filter')
            wl = ws_params.get('wanted_list_filter') or []
        elif count == 2:
            col_specs = ws_params.get('col_specs_initial')
            wl = ws_params.get('wanted_list_initial') or []
        elif count == 3:
            col_specs = ws_params.get('col_specs')
            wl = ws_params.get('wanted_list') or []
        else:
