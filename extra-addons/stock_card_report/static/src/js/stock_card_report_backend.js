odoo.define('stock_card_report.stock_card_report_backend', function (require) {
    ...
    var report_backend = Widget.extend(ControlPanelMixin, {
        ...
        set_html: function () {
            ...
            def.then(function () {
                self.report_widget.$el.html(html);
            });
        },
        start: function () {
            this.set_html();
        ...
        get_html: function () {
            ...
            var defs = [];
            var def = $.when();
            var new_model = new Model(this.given_context.model);
            new_model.call('get_html', [self.given_context]).done(
				function(report_html){
                    self.html = report_html.html;
...
//                });
        ...
        print: function () {
            var self = this;
            var new_model_print = new Model(this.given_context.model);
            new_model_print.call('print_report', ['qweb-pdf']).done(
				function(report){
				    var active_id = parseInt(/id=(\d+)/.exec(url)[1]);
				    var print = {'context': {'active_id': parseInt(active_id),
        								'active_ids': [parseInt(active_id)]},
...
//                self.do_action(result);
        ...
        export: function () {
            var self = this;
            var new_model_xls = new Model(this.given_context.model);
            new_model_xls.call('print_report', [this.given_context.active_id,'xlsx']).done(
				function(result){
				    self.do_action(result);
//            this._rpc({
//                model: this.given_context.model,
