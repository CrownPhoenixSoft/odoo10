...
class PWA(Controller):
    def get_asset_urls(self, asset_xml_id):
        ...
        assets = qweb._get_asset(asset_xml_id, {}, True, True)
        urls = []
        for asset in assets:
            if asset[0] == "link":
    ...
    def manifest(self):
        ...
        pwa_name = config_param.get_param("pwa.manifest.name", "ClicktoHUB PWA")
        pwa_short_name = config_param.get_param("pwa.manifest.short_name", "ClicktoHUB PWA")
