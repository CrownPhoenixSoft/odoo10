...
class WizardValuationStockInventory(models.TransientModel):
    ...
    def print_pdf_stock_inventory(self,data):
        ...
        for wizard_id in self.env['wizard.valuation.stock.inventory.line'].sudo().search([('wizard_id','=',self.id)]):
        ...
        if data["product_sub_categ_id"]:
            for resource in self.env['stock.quant'].sudo().search([('location_id','=',self.location_id.id)]):
                ...
                if resource.product_id.categ_id.id ==  data["product_sub_categ_id"] or resource.product_id.categ_id.parent_id.id ==  data["product_sub_categ_id"]:
                    if self.env.user.company_id.id in resource.product_id.company_ids.ids and self.env.user.company_id.id == resource.company_id.id:
