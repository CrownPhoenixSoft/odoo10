...
WHERE state = 'done'
  and spo.id NOT IN (SELECT id FROM error_move_lines)
...
FROM stock_pack_operation spo
         NATURAL JOIN product
         JOIN location ON (spo.location_id = location.location_id)
WHERE state = 'done'
  AND spo.id NOT IN (SELECT id FROM error_move_lines)
GROUP BY product_id, spo.location_id, usage, package_id, result_package_id
"""
...
class AlignMoveQuantWizard(models.Model):
    ...
    def _default_start_date(self):
        ...
        first_move_line = self.env["stock.pack.operation"].search(
            [], order="create_date", limit=1
        )
        # print(first_move_line)
        if first_move_line:
            print(first_move_line.create_date)
            # what's wrong in following warning raised is AttributeError: 'str' object has no attribute 'strftime'
            # return fields.Datetime.to_string(first_move_line.create_date)
            return first_move_line.create_date
            # return fields.Datetime.to_string(first_move_line.create_date)
        else:
            return False
