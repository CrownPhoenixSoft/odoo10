...
class TemplateSettings(models.Model):
    ...
    aiw_report = fields.Boolean('Display amount in words?', default=True, help="Check this box to enable the display of amount in words in the invoice/quote/sale order reports")
    ...
    show_footer_logo = fields.Boolean('Display footer logo?', default=True, help="Check this box to display footer logo in the reports")
    transparent = fields.Boolean('Have transparent background?', default=False, help="Check this box to have a transparent background for your reports. This is useful when the watermark feature is enabled so that your watermark is not hidden behind the report content/text")
    footer = fields.Boolean('Enable report footer?', default=True, help="Check this box to enable footer in your reports. You may want to disable footer if you are using a watermark PDF with a footer content already")
    header = fields.Boolean('Enable report header?', default=True, help="Check this box to enable header in your reports. You may want to disable header if you are using a watermark PDF with a header content already")
    ############ Watermark Settings ################
...
''', help="Put a python expression/code which when evaluated, will result in some text to be printed on PDF as watermark.")
    ...
    def wm_eval(self, doc, time, user, company):
        ...
        #os.environ['TZ'] = self._context.get('tz') or self.env.user.tz or 'UTC' #set correct timezone
        #time.tzset()
