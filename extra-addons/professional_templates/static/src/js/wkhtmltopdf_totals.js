 ...
 */
var pdfPage = {
	width: 8.26771654, // inches, 210mm //A4 Paper Size
	height: 11.6929134, // inches, 297mm//A4 Paper Size
	margins: {
...
$(window).load(function () {
	...
	var pageHeight = Math.floor(
        ...
        var $totals_div = $('.'+totalsClassName)
        var topPosition = $totals_div.position().top;
        var leftPosition = $totals_div.position().left;
        var totalsHeight = $totals_div.height();
        var totalsWidth = $totals_div.width();
        var offSet = (topPosition % pageHeight) - totalsHeight;
        $totals_div.css('padding-top', offSet)	
	...
	if (visualDebug) {
		$body.append('<div id="debug_div" style="position: absolute; top: 0; height:' + (pageHeight - 2) + 'px;width:150px; ' + 
				'right: 0; border: 1px solid #FF0000; background: blue; color: white;">
                                Test<br />' + pageHeight + 
                                '<br />TOP:'+ topPosition +
                                '<br />LEFT:'+ leftPosition + 
                                '<br />H:'+ totalsHeight + 
                                '<br />W:'+ totalsWidth + 
                                '<br /></div>');
