...
class KsDashboardNinjaItems(models.Model):
    ...
    ks_background_color = fields.Char(default="#ffffff,0.99", string="Background Color")
    ...
    ks_record_field = fields.Many2one('ir.model.fields',
                                      ...
                                      string="Record Field", strore=True)
    ...
    ks_dashboard_item_type = fields.Selection([('ks_tile', 'Tile'),
                                              ...
                                              required=True, string="Dashboard Item Type")
    ...
    ks_target_view = fields.Selection([('Number','Number'),('Progress Bar','Progress Bar')],string="View", default="Number")
    ks_data_comparison = fields.Selection([('None', 'None'),
                                               ('Sum', 'Sum'),
                                               ('Ratio', 'Ratio'),
                                               ('Percentage', 'Percentage')], string="Data Type", default="None")
    ...
    ks_record_field_2 = fields.Many2one('ir.model.fields',
                                        ...
                                        string="Record Field",  strore=True)
    ks_record_count_2 = fields.Float(string="KPI Record Count", compute='ks_get_record_count_2',store=False)
    ...
    ks_goal_enable = fields.Boolean(string="Enable Target", default=False)
        ...
        for rec in self:
            if (rec.ks_chart_date_groupby in ('minute', 'hour') or rec.ks_chart_date_sub_groupby in ('minute', 'hour')) \
                ...
                raise ValidationError(_(
                    "Cannot set aggregation having Date time (Hour, Minute) when target lines are being used."
    ...
    def ks_date_target_line_onchange(self):
        for rec in self:
            if rec.ks_chart_date_groupby in ('minute', 'hour') or rec.ks_chart_date_sub_groupby in ('minute', 'hour'):
                rec.ks_goal_lines = False
                return {'warning': {
                    'title': _('Groupby Field aggregation'),
                    'message': _('Cannot create target lines for Groupby field having aggregation time in  minute and hour')
                }}

    @api.multi
    @api.constrains('ks_chart_date_groupby', 'ks_chart_date_sub_groupby')
    def ks_date_target(self):
        for rec in self:
            if (rec.ks_chart_date_groupby in ('minute', 'hour') or rec.ks_chart_date_sub_groupby in ('minute', 'hour')) \
                    and rec.ks_goal_lines:
                raise ValidationError(_("Cannot set aggregation having time, you currently have target line."
                                        " To proceed first delete target lines"))


    @api.multi
    @api.onchange('ks_chart_date_groupby', 'ks_chart_date_sub_groupby')
    def ks_date_target_onchange(self):
        for rec in self:
            if (rec.ks_chart_date_groupby in ('minute', 'hour') or rec.ks_chart_date_sub_groupby in ('minute', 'hour')) \
                    and rec.ks_goal_lines:
                raise ValidationError(_("Cannot set aggregation having time, you currently have target line."
    ...
    @api.constrains('ks_chart_relation_groupby')
    def ks_check_chart_relation_group_by(self):
        if self.ks_chart_relation_groupby.ttype == 'date' and self.ks_chart_date_groupby in (
                'minute', 'hour'):
            raise ValidationError(_('Groupby field: {} cannot be aggregated by {}').format(
                self.ks_chart_relation_groupby.display_name, self.ks_chart_date_groupby))

    @api.constrains('ks_chart_relation_sub_groupby')
    def ks_check_chart_relation_sub_groupby(self):
        if self.ks_chart_relation_sub_groupby.ttype == 'date':
            if self.ks_chart_date_sub_groupby in ('minute', 'hour'):
                raise ValidationError(_('Sub Groupby field: {} cannot be aggregated by {}').format(
                    self.ks_chart_relation_sub_groupby.display_name, self.ks_chart_date_sub_groupby))
    ...
    @api.depends('ks_chart_measure_field', 'ks_chart_relation_groupby', 'ks_chart_date_groupby', 'ks_domain',
                 ...
                 'ks_goal_bar_line', 'ks_semi_circle_chart','ks_show_data_value','ks_item_start_date',
                 'ks_item_end_date','ks_compare_period', 'ks_year_period','ks_chart_item_color','ks_bar_chart_stacked',
    ...
    def ks_get_chart_data(self):
        for rec in self:
            ...
            # if not rec.ks_dashboard_item_type == 'ks_bar_chart':
            ...
            if rec.ks_dashboard_item_type and rec.ks_dashboard_item_type != 'ks_tile' and rec.ks_dashboard_item_type != 'ks_list_view' and rec.ks_model_id and rec.ks_chart_data_count_type:
                ks_chart_data = {'labels': [], 'datasets': [], 'ks_show_second_y_scale': False, 'domains': [],
                                 'ks_selection': "", 'ks_currency': 0, 'ks_field': "",
                                 'ks_chart_type': rec.ks_dashboard_item_type,
                ...
                if ((rec.ks_chart_data_count_type != "count" and ks_chart_measure_field) or (
                        ...
                        if rec.ks_chart_groupby_type == 'date_type' and rec.ks_goal_enable and rec.ks_dashboard_item_type in [
                            ...
                            if rec.ks_goal_lines and len(rec.ks_goal_lines) != 0:
                                ...
                                if rec.ks_date_filter_selection_2 == "l_none":
                                    ...
                                    ksDateFilterSelection = rec._context.get('ksDateFilterSelection')
                                else:
                                ...
                                if selected_start_date and selected_end_date:
                                    if ksDateFilterSelection == 'l_custom':
                                        ...
                                        ks_goal_domain.extend([('ks_goal_date', '>=',
                                                                str(datetime.strptime(str(selected_start_date),
                                                                                      DEFAULT_SERVER_DATETIME_FORMAT))),
                                                               ('ks_goal_date', '<=',
                                                                str(datetime.strptime(str(selected_end_date),
                                                                                      DEFAULT_SERVER_DATETIME_FORMAT)))])
                                    else:
                                        ks_goal_domain.extend([('ks_goal_date', '>=', str(selected_start_date.date())),
                                                               ('ks_goal_date', '<=', str(selected_end_date.date()))])
                    ...
                    if rec.ks_chart_relation_sub_groupby.ttype == 'date':
                        ...
                        ks_chart_date_sub_groupby, ks_chart_date_groupby = (
                            rec.ks_chart_date_sub_groupby, rec.ks_chart_date_groupby)  # doesn't have time in date
                    else:
                ...
                if ks_chart_data and len(ks_chart_data['datasets']) == 0:
                    data = {'data': [],
                            'label': []}
    ...
    def ks_fetch_chart_data(self, ks_model_name, ks_chart_domain, ks_chart_measure_field, ks_chart_measure_field_2,
        ...
        for res in ks_chart_records:
            ...
            if res[ks_chart_groupby_field] and all(
                ...
                if ks_chart_groupby_type == "relational_type":
                    ks_chart_data['labels'].append(res[ks_chart_groupby_field][1])
        ...
        for rec in self:
            if rec.ks_list_view_type and rec.ks_dashboard_item_type and rec.ks_dashboard_item_type == 'ks_list_view' and \
                ...
                ks_list_view_data = {'label': [],
                                     'data_rows': [], 'model': rec.ks_model_name, 'date_index': []}
    ...
    def get_target_list_view_data(self, ks_list_view_records, rec, ks_list_fields, ks_group_by,
        ...
        if rec.ks_date_filter_selection == "l_none":
            if rec._context.get('ksDateFilterSelection') == 'l_custom':
                selected_start_date = datetime.strptime(str(rec._context.get('ksDateFilterStartDate')),
                                                        DEFAULT_SERVER_DATETIME_FORMAT)
                selected_end_date = datetime.strptime(str(rec._context.get('ksDateFilterEndDate')),
                                                      DEFAULT_SERVER_DATETIME_FORMAT)
        ...
        if selected_start_date and selected_end_date:
            ks_goal_domain.extend(
                [('ks_goal_date', '>=', selected_start_date.date().strftime(DEFAULT_SERVER_DATE_FORMAT)),
    ...
    @api.constrains('ks_domain')
    def ks_onchange_check_domain(self):
        if self.ks_domain:
            proper_domain = []
            try:
                ks_domain = self.ks_domain
    ...
    def ks_convert_into_proper_domain(self, ks_domain, rec):
        if ks_domain and "%UID" in ks_domain:
            ks_domain = str(eval(ks_domain)).replace("'%UID'", str(self.env.user.id))
        ...
        if ks_data:
            ...
            for data in ks_data:
                if field_type == 'date_type':
                    if ks_chart_date_groupby == 'day' and ks_chart_date_sub_groupby in ["quarter", "year"]:
    ...
    def set_color_palette(self):
        for rec in self:
            if rec.ks_dashboard_item_type == "ks_bar_chart" or rec.ks_dashboard_item_type == "ks_horizontalBar_chart" or rec.ks_dashboard_item_type == "ks_line_chart" or rec.ks_dashboard_item_type == "ks_area_chart":
                ...
                rec.ks_font_color = "default"
                rec.ks_default_icon_color = "default"
    ...
    def ks_set_date_filter(self):
        for rec in self:
            if (not rec.ks_date_filter_selection) or rec.ks_date_filter_selection == "l_none":
                ...
                ks_date_data = ks_get_date(str(rec.ks_date_filter_selection))
    ...
    @api.depends('ks_dashboard_item_type', 'ks_model_id', 'ks_model_id_2', 'ks_record_field', 'ks_goal_enable',
                 ...
                 'ks_compare_period', 'ks_year_period', 'ks_item_start_date_2', 'ks_item_end_date_2')
    ...
    def ks_convert_into_proper_domain_2(self, ks_domain_2, rec):
        ...
        if (not rec.ks_date_filter_selection_2) or rec.ks_date_filter_selection_2 == "l_none":
            ...
            if rec.ks_item_start_date_2 and rec.ks_item_end_date_2:
                selected_start_date = datetime.strptime(rec.ks_item_start_date_2, DEFAULT_SERVER_DATETIME_FORMAT)
                selected_end_date = datetime.strptime(rec.ks_item_end_date_2, DEFAULT_SERVER_DATETIME_FORMAT)
        else:
            if rec.ks_date_filter_selection_2 and rec.ks_date_filter_selection_2 != 'l_custom':
                selected_start_date = ks_get_date(rec.ks_date_filter_selection_2)['selected_start_date']
                selected_end_date = ks_get_date(rec.ks_date_filter_selection_2)['selected_end_date']
        ...
        if ks_domain_2:
            # try:
            proper_domain = safe_eval(ks_domain_2)
            ...
            if selected_start_date and selected_end_date and rec.ks_date_filter_field_2:
    ...
    def format_label(self, value, ftype, display_format, tz_convert, locale):
        ...
        if ftype == 'datetime':
            ...
            if tz_convert:
                value = pytz.timezone(self._context['tz']).localize(value)
                tzinfo = value.tzinfo
            return babel.dates.format_datetime(value, format=display_format, locale=locale)
    ...
    def ks_get_start_end_date(self, model_name, ks_chart_groupby_relation_field, ttype, ks_chart_domain,
        ...
        if model_field_start_date and goal_model_start_date:
            ...
            if datetime.strptime(model_field_start_date, '%Y-%m-%d %H:%M:%S') < datetime.strptime(goal_model_start_date,
                                                                                                  '%Y-%m-%d %H:%M:%S'):
                ks_start_end_date['start_date'] = model_field_start_date
            else:
                ks_start_end_date['start_date'] = goal_model_start_date
            if datetime.strptime(model_field_end_date, '%Y-%m-%d %H:%M:%S') > datetime.strptime(goal_model_end_date,
                                                                                                '%Y-%m-%d %H:%M:%S'):
...
class KsDashboardItemsGoal(models.Model):
    ...
    ks_goal_date = fields.Date(string="Date", required=True)
    ks_goal_value = fields.Float(string="Value")

    ks_dashboard_item = fields.Many2one('ks_dashboard_ninja.item', string="Dashboard Item")

    @api.constrains('ks_goal_date','ks_goal_value')
    def _ks_chaeck_target_line(self):
        for rec in self:
            if rec.ks_dashboard_item.ks_chart_date_groupby in ('minute', 'hour') or \
                    rec.ks_dashboard_item.ks_chart_date_sub_groupby in ('minute', 'hour'):
                raise ValidationError(_('Cannot create target lines for Groupby field having aggregation time in '
                                        'minute and hour'))
...
class KsDashboardItemsActions(models.Model):
    ...
    def ks_check_date_group_by_onchange(self):
        for rec in self:
    ...
    @api.constrains('ks_item_action_date_groupby')
