...
class KsDashboardNinjaBoard(models.Model):
    ...
    ks_dashboard_top_menu_id = fields.Many2one('ir.ui.menu', domain="[('parent_id','=',False)]", string="Show Under Menu")
    ...
    def create(self, vals):
        ...
        if 'ks_dashboard_top_menu_id' in vals and 'ks_dashboard_menu_name' in vals:
            action_id = {
                ...
                # 'context': "{'form_view_ref':'ks_dashboard_ninja.board_dashboard'}",
            }
    ...
    def write(self, vals):
        ...
        for rec in self:
            if 'ks_dashboard_menu_name' in vals:
                # Default dashboard check condition
                if self.env.ref('ks_dashboard_ninja.ks_my_default_dashboard_board',False) and self.env.ref('ks_dashboard_ninja.ks_my_default_dashboard_board').id == rec.id:
                    ...
                    self.env.ref('ks_dashboard_ninja.my_dashboard').sudo().name = vals['ks_dashboard_menu_name']
                else:
                    rec.ks_dashboard_menu_id.sudo().name = vals['ks_dashboard_menu_name']
                # rec.ks_dashboard_menu_id.name = vals['ks_dashboard_menu_name']
            if 'ks_dashboard_group_access' in vals:
                if self.env.ref('ks_dashboard_ninja.ks_my_default_dashboard_board',False) and self.env.ref('ks_dashboard_ninja.ks_my_default_dashboard_board').id == rec.id:
                    self.env.ref('ks_dashboard_ninja.board_menu_root').sudo().groups_id = vals['ks_dashboard_group_access']
                    self.env.ref('ks_dashboard_ninja.my_dashboard').sudo().groups_id = vals['ks_dashboard_group_access']
                else :
    ...
    def unlink(self):
        if self.env.ref('ks_dashboard_ninja.ks_my_default_dashboard_board').id in self.ids:
            raise ValidationError("Default Dashboard can't be deleted.")
        else:
            for rec in self:
    ...
    @api.model
    def ks_is_dashboard_manager(self,user_id):
        if self.env.ref('ks_dashboard_ninja.ks_dashboard_ninja_group_manager') in self.env['res.users'].browse(user_id).groups_id:
            return True
        return False
    ...
    def ks_import_dashboard(self, file):
        ...
        for data in ks_dashboard_data:
            ...
            if data['ks_gridstack_config']:
                ks_gridstack_config = eval(data.get('ks_gridstack_config'))
            ...
            if data['ks_item_data']:
                ...
                for item in data['ks_item_data']:
                    ...
                    ks_goal_lines =deepcopy(item['ks_goal_liness']) if item.get('ks_goal_liness', False) else False
                    ks_action_lines =deepcopy(item['ks_action_liness']) if item.get('ks_action_liness', False) else False
    ...
    @api.model
    def ks_get_my_dashboard_item_view(self, ks_item_id):
        rec = self.env['ks_dashboard_ninja.item'].browse(int(ks_item_id))
        return {'domain_model': rec.ks_model_name, 'domain_value': rec.ks_domain, }


    @api.model
    def ks_delete_dashboard_item(self, ks_item_id):
        self.env['ks_dashboard_ninja.item'].browse(ks_item_id).unlink()
        return "Deleted"



    @api.model
    def ks_save_dashboard_name(self, ks_new_dashboard_name,ks_dashboard_id):
        self.browse(ks_dashboard_id).name = ks_new_dashboard_name
