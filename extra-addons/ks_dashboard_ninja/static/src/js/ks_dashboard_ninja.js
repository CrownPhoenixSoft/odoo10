    ...
    var KsDashBoardNinja = Widget.extend(ControlPanelMixin, {
       ...
       events: {
        ...
        init: function (parent,state, params) {
            ...
            this.ks_date_filter_selection_order = ['l_day','t_week','t_month','t_quarter','t_year','n_day','n_week',
                                                    'n_month','n_quarter','n_year','ls_day','ls_week','ls_month',
                                                    'ls_quarter','ls_year','l_week','l_month','l_quarter','l_year','l_custom'];
       ...
       willStart: function() {
        ...
        ksUpdateDashboardItem : function (items) {
            ...
            for (var i=0;i<items.length;i++){
                ...
                if (item_data['ks_dashboard_item_type'] == 'ks_list_view'){
                    ...
                    $(self.$el.find(".grid-stack-item[data-gs-id="+item_data.id+"]").children()[0]).find(".card-body").empty();
                    self._renderChart($(self.$el.find(".grid-stack-item[data-gs-id="+item_data.id+"]").children()[0]),item_data);

                    if (!self.ks_dashboard_data.ks_item_data[item_data.id]['isDrill']){
                       $(self.$el.find(".grid-stack-item[data-gs-id="+item_data.id+"]")).find('.ks_dashboard_item_drill_up').addClass('ks_d_none');
                    }
        ...
        onKsDashboardMenuContainerHide: function (e) {
         ...
         on_detach_callback : function(){
            ...
            self.ks_fetch_data();
            $('.o_content').removeClass('ks_white');
        },

        ksFetchUpdateItem: function(id){
            var self = this;
            var item_data = self.ks_dashboard_data.ks_item_data[id];
            return new Model('ks_dashboard_ninja.board').call('ks_fetch_item',[[item_data.id], self.ks_dashboard_id],{'context':self.getContext()}).then(function(new_item_data){
...
//        Have  to save dashboard here
        ...
        ks_fetch_data: function () {
            ...
            return self.ksCurrentDashboardModel.call('ks_fetch_dashboard_data', [self.ks_dashboard_id],{'context':self.getContext()}).done(function(result){
                    ...
                    else if (items[i].ks_dashboard_item_type === 'ks_kpi')
                    {
                       var $kpi_preview = self.renderKpi(items[i])
                        ...
                        if (item_id in self.gridstackConfig) {
                            self.grid.addWidget($kpi_preview, self.gridstackConfig[item_id].x, self.gridstackConfig[item_id].y, self.gridstackConfig[item_id].width, self.gridstackConfig[item_id].height, false, 8, null, 2, 4, item_id);
                        } else {
                           self.grid.addWidget($kpi_preview, 0, 0, 8, 2, true, 8, null, 2, 4, item_id);
                        }
                    }
                    else
                    {
       ...
       renderKpi : function(item, grid){
            ...
            if (field['ks_icon_select'] == "Custom") {
                if (field['ks_icon'][0]) {
                    ks_icon_url = 'data:image/' + (self.file_type_magic_word[field.ks_icon] || 'png') + ';base64,' + field['ks_icon'];
                } else {
                    ks_icon_url = false;
                }
            }
            ...
            var item_info = {
                ...
                title_name : field['name'] ? field['name'] : field['ks_model_id.data.display_name'],
            ...
            if(!kpi_data[1]){
                ...
                if($kpi_preview.find('.ks_target_previous').children().length !== 2){
                    $kpi_preview.find('.ks_target_previous').children().addClass('ks_text-center');
                }
            }
            else{
                switch(field['ks_data_comparison']){
                    case "None":
                        ...
                        item_info['ks_inner_container_class']= ks_inner_container_class;
                    ...
                    case "Sum":
                        ...
                        var target_progress_deviation = target_deviation && target_deviation !== Infinity ? Math.abs(100 - target_deviation) : 0
                    ...
                    case "Percentage":
                        ...
                        item_info['count'] = count ? formats.format_value(count, { type: 'integer'})+"%" : "0%";
        ...
        _ksRenderDashboardTile: function (tile){
            ...
            var count = formats.format_value(tile.ks_record_count, { type: 'float', digits: [69, 2]});
                ...
                switch (tile.ks_layout) {
                   ...
                   ks_rgba_default_icon_color = self._ks_get_rgba_format(tile.ks_default_icon_color);
                     ...
                     default:item_view = QWeb.render('ks_dashboard_item_layout_default', {
                        item: tile,
                        ks_dashboard_list: self.ks_dashboard_data.ks_dashboard_list,
                    });
                    break;
        ...
        ksRenderDashboard: function () {
            var self = this;
            self.$el.empty();
            self.$el.addClass('ks_dashboard_ninja flex-column');
        ...
        ksSortItems: function(ks_item_data){
         ...
         _ksRenderNoItemView: function () {
            $('.ks_dashboard_items_list').remove();
            var self = this;
            $(QWeb.render('ks_no_content_default_view')).appendTo(self.$el)
        },
        ...
        on_reverse_breadcrumb : function(state){
            ...
            if (this.getParent() && this.getParent().do_push_state) {
                 return $.when(self.ks_fetch_data()).then(function(){
                    self.getParent().do_push_state({});
                });
            }
        },
        ...
        _renderChart($ks_gridstack_container,item){
           ...
           $ks_gridstack_container.find('.ks_color_menu').data({chartType:chart_type,chartFamily:chart_family});
            ...
            if (ksMyChart.config.data.datasets.length){
                self.ksChartColors(item.ks_chart_item_color, ksMyChart, chart_type, chart_family,item.ks_bar_chart_stacked,item.ks_semi_circle_chart,item.ks_show_data_value,chart_data);
            ksMyChart.update();
            }
        },
        ...
        onKsDuplicateItemClick: function (e) {
            ...
            new Model('ks_dashboard_ninja.item').call('copy',[parseInt(ks_item_id), {
        ...
        // for rendering the list view in Dashboard
         _renderListView: function (item, grid) {
            ...
            if (item_id in self.gridstackConfig) {
                grid.addWidget($ks_gridstack_container, self.gridstackConfig[item_id].x, self.gridstackConfig[item_id].y, self.gridstackConfig[item_id].width, self.gridstackConfig[item_id].height, false, 9, null, 4, null, item_id);
            } else {
                grid.addWidget($ks_gridstack_container, 0, 0, 13, 4, true, 9, null, 4, null, item_id);
            }
        },
       ...
       //this will delete item from dashboard
        ...
        ks_delete_item: function (id, item) {
            ...
            Dialog.confirm(this, (_t("Are you sure you want to remove this item?")), {
                confirm_callback: function () {
                    new Model('ks_dashboard_ninja.item').call('unlink',[id]).then(function (data) {
        ...
        ksOnListItemInfoClick: function (e) {
            ...
            if(e.currentTarget.dataset.listType==="other"){
      ...
      _onKsClearDateValues: function () {
        ...
        _checkDateFields:function(){
            if(!($("#ksstartdatepicker").val())){
                $("#ksstartdatepicker").val($("#ksenddatepicker").val());
                $("#ksActualStartDateToStore").val($("#ksActualEndDateToStore").val());
            }
            if(!($("#ksenddatepicker").val())){
                $("#ksenddatepicker").val($("#ksstartdatepicker").val());
         ...
         _renderDateFilterHeader:  function () {
           ...
           this.ksStartDatePickerWidget = new (datepicker.DateTimeWidget)(this);
            this.ksStartDatePickerWidget.appendTo(self.$el.find(".ks_date_input_fields")).then((function () {
                ...
                this.ksStartDatePickerWidget.set_value(startDate);
            ...
            this.ksEndDatePickerWidget.appendTo(self.$el.find(".ks_date_input_fields")).then((function () {
                ...
                this.ksStartDatePickerWidget.$el.find("input").attr("placeholder", "Start Date");
                this.ksEndDatePickerWidget.set_value(endDate);
    ...
    _onKsApplyDateFilter: function (e) {
            ...
            var start_date = self.ksStartDatePickerWidget.get_value();
            var end_date = self.ksEndDatePickerWidget.get_value();
            if (start_date === "Invalid date") {
                ...
                if (start_date && end_date) {
                    if (typeof(start_date)=='object'){
                        start_date = start_date.format("YYYY-MM-DD H:m:s");
                    }
                    if (typeof(end_date)=='object'){
                        end_date = end_date.format("YYYY-MM-DD H:m:s");
                    }
                    if (start_date <= end_date) {

                        self.ksDateFilterSelection = self.$el.find('.ks_date_filter_selected').attr('id');
                        self.ksDateFilterStartDate = new moment(start_date).add(new Date(start_date).getTimezoneOffset(), 'minutes');
                        self.ksDateFilterEndDate = new moment(end_date).add(new Date(end_date).getTimezoneOffset(), 'minutes');
        ...
        _ksShowApplyClearDateButton: function () {
            if ($("#ksstartdatepicker").val() && $("#ksenddatepicker").val()) {
...
//            console.log("XLS");
        ...
        ksOnDrillUp : function(e){
            ...
            if (item_data.sequnce){
                ...
                if(sequnce >= 0){
                new Model('ks_dashboard_ninja.item').call('ks_fetch_drill_down_data',[item_id, domain, sequnce]).then(function(result){
                            ...
                            $(self.$el.find(".grid-stack-item[data-gs-id="+item_id+"]").children()[0]).find(".ks_dashboard_item_drill_up").removeClass('ks_d_none');
                ...
                self.ksFetchUpdateItem(item_id).then(function(result){
                    $(self.$el.find(".grid-stack-item[data-gs-id="+item_id+"]")).find('.ks_dashboard_item_drill_up').addClass('ks_d_none');
                });
                var updateValue = self.ks_dashboard_data.ks_item_data[item_id]["ks_update_items_data"];
                if (updateValue){
                    var updateinterval = setInterval(function(){self.ksFetchUpdateItem(item_id)}, updateValue);
                ...
                $(self.$el.find(".grid-stack-item[data-gs-id="+item_id+"]").children()[0]).find(".ks_dashboard_item_drill_up").addClass('ks_d_none');
            }
        ...
        onKsChartMoreInfoClick : function(evt){
            var self = this;
            var item_id = evt.currentTarget.dataset.itemId;                            delete self.ksUpdateDashboard[item_id];
            var item_data = self.ks_dashboard_data.ks_item_data[item_id];
            var groupBy = item_data.ks_chart_groupby_type!=='date_type'?item_data.ks_chart_relation_groupby_name:item_data.ks_chart_relation_groupby_name+':'+item_data.ks_chart_date_groupby;
            var action = {
                    name: _t(item_data.name),
                    type: 'ir.actions.act_window',
                    res_model: item_data.ks_model_name,
        ...
        onChartCanvasClick : function(evt){
            var self = this;
            var item_id = item_id = evt.currentTarget.dataset.chartId;
            if (item_id in self.ksUpdateDashboard) {
                clearInterval(self.ksUpdateDashboard[item_id]);
            ...
            if (activePoint){
                ...
                if (activePoint._chart.data.domains){
                    ...
                    if (item_data.max_sequnce != 0 && sequnce < item_data.max_sequnce){
                        new Model('ks_dashboard_ninja.item').call('ks_fetch_drill_down_data',[item_id, domain, sequnce])
                            .then(function(result){
                            self.ks_dashboard_data.ks_item_data[item_id]['ks_chart_data'] = result.ks_chart_data;
                            self.ks_dashboard_data.ks_item_data[item_id]['sequnce'] = result.sequence;
                            self.ks_dashboard_data.ks_item_data[item_id]['ks_dashboard_item_type']  = result.ks_chart_type;
                           ...
                           $(self.$el.find(".grid-stack-item[data-gs-id="+item_id+"]").children()[0]).find(".card-body").empty();
                           $(self.$el.find(".grid-stack-item[data-gs-id="+item_id+"]")).find('.ks_dashboard_item_drill_up').removeClass('ks_d_none');
                            var item_data = self.ks_dashboard_data.ks_item_data[item_id]
                            self._renderChart($(self.$el.find(".grid-stack-item[data-gs-id="+item_id+"]").children()[0]),item_data);
