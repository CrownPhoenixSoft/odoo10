...
//            this.chart_type = this.fields.ks_dashboard_item_type.split('_')[1];
         ...
         _renderChart: function(){
            var self = this;
            if(this.fields.ks_chart_measure_field_2.length>0 && this.chart_data.ks_chart_type === 'ks_bar_chart'){
                var scales  = {}
                scales.yAxes = [
                    {
                        type: "linear",
                ...
                var self = this;
                var $chartContainer = this.$el.find('#ksMyChart');
                var chart_plugin = [];
                if (this.fields.ks_show_data_value) {
                    chart_plugin.push(ChartDataLabels);
                }
                if (this.chart_type !== "kpi" ){this.ksMyChart = new Chart($chartContainer[0], {
