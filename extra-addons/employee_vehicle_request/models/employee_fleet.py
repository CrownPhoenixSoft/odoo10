...
class FleetVehicleInherit(models.Model):
    ...
    reserved_time = fields.One2many('fleet.reserved', 'reserved_obj', String='Reserved Time', readonly=1,
                                    ...
                                    ondelete='cascade')
    #-------Customization Pandora
    trip_count=fields.Integer(compute="_compute_count_all_trips", string='Trips History')

    def _compute_count_all_trips(self):
        
        
        for record in self:
            
            record.trip_count=self.env['employee.fleet'].search_count([('fleet','=',record.id)])
    ...
    def action_view_vehicle_request(self):
        ...
        action = self.env.ref('employee_vehicle_request.action_employee_fleet')
